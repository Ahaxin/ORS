...
class ProviderRouter:
    ...
    def get_provider(self, name: str | None = None) -> LLMProvider:
        ...
        match name:
            case "openai":    return OpenAIProvider(api_key=cfg["api_key"], model=cfg["default_model"])
            case "anthropic": return AnthropicProvider(api_key=cfg["api_key"], model=cfg["default_model"])
            case "gemini":    return GeminiProvider(api_key=cfg["api_key"], model=cfg["default_model"])
            case "lmstudio":  return LMStudioProvider(base_url=cfg["base_url"], model=cfg["default_model"])
